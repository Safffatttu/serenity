...
ErrorOr<void> PerformanceEventBuffer::to_json_impl(Serializer& object) const
{
    ...
    for (size_t i = 0; i < m_count; ++i) {
        ...
        case PERF_EVENT_FILESYSTEM:
            ...
            TRY(event_object.add("start_timestamp"sv, event.data.filesystem.start_timestamp));
            switch (event.data.filesystem.type) {
            case FilesystemEventType::Open: {
                const auto& open = event.data.filesystem.data.open;
                TRY(event_object.add("filesystem_type"sv, "open"sv));
            ...
            case FilesystemEventType::Close: {
                const auto& close = event.data.filesystem.data.close;
                TRY(event_object.add("filesystem_type"sv, "close"sv));
            ...
            case FilesystemEventType::Readv: {
                const auto& readv = event.data.filesystem.data.readv;
                TRY(event_object.add("filesystem_type"sv, "readv"sv));
            ...
            case FilesystemEventType::Read: {
                const auto& read = event.data.filesystem.data.read;
                TRY(event_object.add("filesystem_type"sv, "read"sv));
            ...
            case FilesystemEventType::Pread: {
                const auto& pread = event.data.filesystem.data.pread;
                TRY(event_object.add("filesystem_type"sv, "pread"sv));
